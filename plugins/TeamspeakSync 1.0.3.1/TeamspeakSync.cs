...
namespace PRoConEvents
{
    ...
    class TeamspeakSync : PRoConPluginAPI, IPRoConPluginInterface
    {
            ...
            private void parse(String raw)
            {
                ...
                foreach (string element in raw.Split(' '))
                    if (element.Contains("="))
                    {
                        ...
                        //
                        // A value can have "=" in it, so we only need to split on the first occurrance.
                        Int32 keyDelimIndex = element.IndexOf('=');
                        String key = element.Substring(0, keyDelimIndex);
                        String value = element.Substring(keyDelimIndex + 1);
                        if (tsPairs.ContainsKey(key))
                        {
                            tsPairs[key] = TeamspeakHelper.ts_UnescapeString(value);
                        }
                        else
                        {
                            tsPairs.Add(key, TeamspeakHelper.ts_UnescapeString(value));
                        }
                    }
            }
        }
        ...
        public static List<TeamspeakSync> TsSync = new List<TeamspeakSync>();
        ...
        public string GetPluginVersion() { return "1.0.3.1"; }
        ...
        public void OnPluginLoaded(String strHostName, String strPort, String strPRoConVersion)
        {
            ...
            // Register Events.
            this.RegisterEvents("OnPlayerJoin", "OnPlayerLeft", "OnPlayerTeamChange", "OnPlayerSquadChange", "OnListPlayers", "OnPunkbusterPlayerInfo", "OnLevelStarted", "OnLevelLoaded", "OnRoundOver");
        ...
        public void playerSwappedTeamsOrSquads(String Name, Int32 TeamId, Int32 SquadId)
        {
            ...
            foreach (MasterClient mstClient in mClientAllInfo)
                if (mstClient.HasGmClient && mstClient.GmClient.Name == Name)
                {
                    ...
                    mstClient.GmClient = new GameClient(
                        new CPlayerInfo(mstClient.GmClient.Name, mstClient.GmClient.Tags, TeamId, SquadId),
                        mstClient.GmClient.PunkbusterInfo);
                    foreach (GameClient gmClient in mClientGmInfo)
                        if (gmClient.Name == Name)
                        {
                            mClientGmInfo.Remove(gmClient);
